...
contract Back 
{
    ...
    function testOnly(uint borrower_id, uint book_id) public returns(int8)
    {
        Book memory book = Books[book_id];
        		
        book.State = 1;
        book.BorrowerId = borrower_id;
        book.LastBorrowTime = (((now/1000 days)*1000 days) - 70000 days);//借出时间精确到日
        Books[book_id] = book;
    ...
    */
    function insertBook(uint book_id, bytes name, uint256 price) public returns(int8)
    {
        ...
        if(Books[book_id].isUsed || book_id == 0)
        {
            return -1;
        }
    ...
    */
    function updateBook(uint book_id, uint borrower_id, uint new_state) internal returns(int8)
    {
        ...
        if(new_state == 0)
        {
            uint borrow_length = book.LastBorrowTime - (now/1000 days)*1000 days;
            if(borrow_length > 60000 days)
            {
                //超时罚款一天5元
                uint fine = (borrow_length - 60000 days)/1000 days * 5;
    ...
    */
    function insertUser(uint user_id, bytes name) public returns(int8)
    {
        ...
        if(Users[user_id].isUsed || user_id == 0)
        {
            return -1;
        }
    ...
    */
    function returnBook(uint borrower_id, uint book_id) public returns(int8)
    {
	    ...
	    int8 temp = updateBook(book_id,borrower_id,0);
	    if(temp == 0 || temp == 1)
	    {
	        return temp;
	    }
