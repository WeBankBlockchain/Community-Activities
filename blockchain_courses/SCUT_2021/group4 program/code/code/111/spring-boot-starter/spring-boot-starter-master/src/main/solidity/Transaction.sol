...
contract NTFMarket is ParallelContract{
    function ntfOwner(address sender,uint _ntfId) external view returns (address);
    function changeOwnership( uint _ntfId,address _sender) external;
}

contract Transaction is ParallelContract{
    NTFMarket nftContract;
  ...
  */
  address sender;
    ...
    function getMyRecords() public view returns (uint[]){
        uint[] memory myRecord = new uint[](myRecordCount[sender]);
        uint count = 0;
        for(uint i = 0; i < records.length; i++){
            if(records[i].buyer == sender||records[i].seller == sender){
                myRecord[count] = i;
                count++;
            }
        }
...
//查看该商品的某次交易
    ...
    function requestArbitration(uint _recordId, string _reason) public {
        ...
        require(nftContract.ntfOwner(msg.sender,records[_recordId].ntfId) == records[_recordId].buyer, "nft does not belong to you!");
    ...
    function approveArbitration(uint _recordId) public  {
        ...
        uint _ntfId = records[_recordId].ntfId;
        accountContract._transfer( _seller, _buyer, _price);
        nftContract.changeOwnership( _ntfId, _seller);
